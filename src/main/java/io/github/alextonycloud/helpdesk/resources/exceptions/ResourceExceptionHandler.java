...
public class ResourceExceptionHandler {
	...
	public ResponseEntity<StandardError> dataIntegrityViolationException(AccessDeniedException ex, HttpServletRequest request){
		
		StandardError error = new StandardError(System.currentTimeMillis(), HttpStatus.FORBIDDEN.value(), "Permissão negada", ex.getMessage(), request.getRequestURI());
		return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(error);
	}
